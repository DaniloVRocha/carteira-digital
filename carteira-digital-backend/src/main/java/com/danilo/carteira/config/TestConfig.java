...
public class TestConfig implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		OperacaoConta op1 = new OperacaoConta(null, LocalDateTime.now(),LocalDateTime.now(), 'R', 20.00, EstadoPagamento.QUITADO, con1, "Futebol", Categoria.LAZER);
		
		OperacaoConta op2 = new OperacaoConta(null, LocalDateTime.now(),LocalDateTime.now(), 'R', 20.00, EstadoPagamento.QUITADO, con1, "Futebol", Categoria.LAZER);

		OperacaoConta op3 = new OperacaoConta(null, LocalDateTime.now(),LocalDateTime.now(), 'R', 20.00, EstadoPagamento.QUITADO, con1, "Futebol", Categoria.LAZER);

		OperacaoConta op4 = new OperacaoConta(null, LocalDateTime.now(), LocalDateTime.now(), 'R', 40.00, EstadoPagamento.QUITADO, con2, "teste", Categoria.GERAL);

		OperacaoConta op5 = new OperacaoConta(null, LocalDateTime.now(), LocalDateTime.now(), 'R', 40.00, EstadoPagamento.QUITADO, con2, "pix jantar", Categoria.ALIMENTACAO);

		OperacaoConta op6 = new OperacaoConta(null, LocalDateTime.now(),LocalDateTime.now(), 'D', 40.00, EstadoPagamento.QUITADO, con2, "pix estacionamento", Categoria.GERAL);
